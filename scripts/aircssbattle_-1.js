...
function setUpScreen()
{
	...
	screen_div.setAttribute( "id", "game_screen" );
	screen_div.style.width = screen_width;
	screen_div.style.height = screen_height + 25;
	screen_div.style.overflow = "hidden";
	screen_div.style.position = "absolute";
	screen_div.style.left="10px";
	screen_div.style.top = "10px";
	...
	for( var i = 0; i < colBars.length - 1; i++ )
	{
		...
		tmpBar.style.width = screen_width;
		tmpBar.style.height = gap_height + "px";
		tmpBar.style.position = "absolute";
		tmpBar.style.top = screen_height - ( gap_height * ( i + 1 ) );
	...
	var tmpBar = document.createElement( "div" );
	tmpBar.setAttribute( "id", "control" );
	tmpBar.style.width = screen_width;
	tmpBar.style.height = 22 + "px";
	tmpBar.style.position = "absolute";
	tmpBar.style.top = screen_height;
	tmpBar.style.paddingTop = "2px";

	screen_div.appendChild( tmpBar );

	tmpBar.innerHTML = "<div class='sprite' style='margin-left:110px;float:left;display:inline;width:20px;height:20px;background-position:-68px -58px;' onclick='fireOne(1);' onmouseover='this.style.cursor=\"pointer\";steerBullet(1);' onmouseout='steerBullet(0)' >&nbsp;</div>";
	tmpBar.innerHTML += "<div class='sprite' style='float:left;display:inline;width:20px;height:20px;background-position:-88px -58px;' onclick='fireOne(0);' onmouseover='this.style.cursor=\"pointer\";'>&nbsp;</div>";
	tmpBar.innerHTML += "<div class='sprite' style='float:left;display:inline;width:20px;height:20px;background-position:-108px -58px;' onclick='fireOne(2);' onmouseover='this.style.cursor=\"pointer\";steerBullet(2);' onmouseout='steerBullet(0)' >&nbsp;</div>";
	
	// Add everything to the body - Away we go!
	document.getElementById("acb_game_screen").appendChild( screen_div );
...
function acb_run()
{
	if ( the_game.state != GAME_END )
	{
		...
		the_game.mainLoop();	
		//run_timer = setTimeout( "run()", run_speed );
	}
}
...
cPlayer.prototype = 
{
	...
	init : function()
	{
		...
		if ( ! document.getElementById(this.id) )
		{
			...
			tmpDiv.style.top = 0 - this.height;
		
			screen_div.appendChild( tmpDiv );
		}
	
		this.div_ref = document.getElementById( this.id );
		this.div_ref.style.width = this.width;
		this.div_ref.style.height = this.height;
	},
	...
	update : function()
	{
		switch( this.state )
		{
			case PLAYER_INIT:
				this.div_ref.style.width = this.width;
				this.div_ref.style.heigth = this.height;
	...
	draw : function()
	{
		this.div_ref.style.left = this.x;
		this.div_ref.style.top = this.y;
	},
...
cBullet.prototype = 
{
	...
	init : function()
	{
		...
		this.div_ref.style.top = 0 - this.height;
		this.div_ref.innerHTML = this.html;
	},
...
function print( p_msg ){ document.getElementById("debugScreen").innerHTML += p_msg; }
function println( p_msg ){ document.getElementById("debugScreen").innerHTML += p_msg + "<br/>"; }
function cls(){ document.getElementById("debugScreen").innerHTML += "";	}
