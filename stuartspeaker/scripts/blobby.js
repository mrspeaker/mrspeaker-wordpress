...
var MainController = {
	...
	init : function()
	{
		...
		document.getElementById("btnJoints").addEventListener("click", function(){ colours.drawJoints = !colours.drawJoints }, false);
		document.getElementById("btnPoints").addEventListener("click", function(){ colours.drawPoints = !colours.drawPoints }, false);
		document.getElementById("btnOutline").addEventListener("click", function(){ colours.drawOutline = !colours.drawOutline }, false);
		document.getElementById("btnFill").addEventListener("click", function(){ colours.drawFill = !colours.drawFill }, false);
		document.getElementById("btnForce").addEventListener("click", function(){ MainController.addForce() }, false);
	...
	addForce : function()
	{
		var xx = ( Math.random( )*30 )-15;
		var yy = ( Math.random( )*70 )-35;
		var vec = new Vector(xx, yy);			
	...
	main : function()
	{
		...
		this.sc3.sc();
		this.draw();
		setTimeout( function(){ MainController.main() }, 40 );
	},
...
function Structure( environment )
{
	...
	this.init = function( )
	{
		...
		// Add some force!
		this.points[Math.floor(Math.random()*this.points.length)].force.add( new Vector(10, -21.5) );
	};
	...
	this.update = function()
	{
		for( var i = 0; i < this.points.length; i++ )
		{
			...
			point.friction = this.env.collision(point.current, point.previous) ? 0.1 : 0.01
		}
	...
	{		
		if( colours.drawOutline || colours.drawFill )
		{
		    ...
		    context.beginPath(); 
		    context.moveTo(this.points[0].current.x * scaleFactor, 
...
function PointMass( currentX, currentY, mass )
{
	...
	this.move = function( dt )
	{
		...
		c = this.current.x; 
		t = (2.0 - this.friction) * c - (1.0 - this.friction) * this.previous.x + a * dtdt;
		...
		c = this.current.y; 
		t = (2.0 - this.friction) * c - (1.0 - this.friction) * this.previous.y + a * dtdt;
		this.previous.y = c; 
		this.current.y = t;
	...
	this.draw = function( ctx, scaleFactor )
	{
		...
		ctx.beginPath(); 
		ctx.arc(this.current.x * scaleFactor,
			this.current.y * scaleFactor,
		colours.pointWidth, 0.0, Math.PI * 2.0, true);
...
function Joint( pointMassA, pointMassB, shortConst, longConst )
{
	...
	this.draw = function(ctx, scaleFactor)
	{
		...
		ctx.beginPath(); 
		ctx.moveTo(this.pointMassAPos.x * scaleFactor, this.pointMassAPos.y * scaleFactor);
	...
	//	ctx.bezierCurveTo((x2 - x1 /2 ) ,50,70,70,this.pointMassBPos.x * scaleFactor, this.pointMassBPos.y * scaleFactor);
		ctx.lineTo(this.pointMassBPos.x * scaleFactor, this.pointMassBPos.y * scaleFactor);
		ctx.closePath();
		ctx.stroke();
	}
}
...
function Vector(x, y)
{
	...
	this.set = function( vector ){ this.x = vector.x; this.y = vector.y; }
	this.add = function( vector ){ this.x += vector.x; this.y += vector.y; }
	this.subtract = function( vector ){ this.x -= vector.x; this.y -= vector.y; }
	this.dotProduct = function( vector ){ return this.x * vector.x + this.y * vector.y }; 
...
window.addEventListener( "load", function(){
	document.getElementById("blobs").addEventListener( "click", function(){
